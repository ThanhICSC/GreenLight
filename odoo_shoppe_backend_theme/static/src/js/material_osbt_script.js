...
ControlPanel.include({
    start: function() {
        ...
        this.$el.on('click', 'a.toogle_control_panel', function(ev) {
            ...
            if ($('.o_control_panel').find('.row:not(:eq(0))').is(':visible'))
                $('.o_control_panel').find('.row:not(:eq(0))').slideUp(500);
            else
                $('.o_control_panel').find('.row:not(:eq(0))').slideDown(500);
        });
...
UserMenu.include({
    ...
    do_update: function () {
        ...
        var fct = function() {
            ...
            return self.alive(func(session.uid, ["name", "company_id", "lang"])).then(function(res) {
                ...
                //     user_lang = 'en_US';
                // }
                // $lang_icon.attr('src', '/odoo_shoppe_backend_theme/static/src/img/flag/'+user_lang+'.png');
...
$(document).ready(function(){
    ...
    $('.toggle-slidebar').on('click', function(){
        ...
        var iconic = leftbar.hasClass('fix_icon_width');
        var is_mobile =  $(window).width() < 768;
        switch_theme_views(!iconic, is_mobile);
    });
    ...
    function checkWidth() {
        ...
        var leftbar = $('div.o_sub_menu');
        switch_theme_views(windowsize < 768, windowsize < 768);
        if (windowsize > 991) {
            $('.o_control_panel').find('.row:not(:eq(0))').slideDown(500);
        }
    }
    // checkWidth();
    var menu_style_cookie = utils.get_cookie('theme_menu_style');
    if(menu_style_cookie){
        var menu_style = JSON.parse(menu_style_cookie);
        switch_theme_views(menu_style.iconic_menu, menu_style.is_mobile);
    }
